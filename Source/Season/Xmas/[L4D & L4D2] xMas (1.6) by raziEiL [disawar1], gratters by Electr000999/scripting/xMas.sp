...
/*========*/
ConVar g_hTimer, g_hSendTo, g_hSnow, g_hNoSnowFix;
Handle g_hxMsg, g_hMsgTimer;
int g_iSpamBlock[MAXPLAYERS+1];
float g_fCvarTimer = 120.0;
int g_iCvarSendTo; 
bool g_bCvarSnow, g_bCvarSnowFix, g_bOn, g_bBlock;
...
public void XM_ev_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	CreateTimer(0.3, XM_t_CreateSnowFall);
}

public Action Command_XmasParty(int client, int agrs)
{
	if (client)
	{
		if (!g_bOn)
...
public Action Command_Xmas(int client, int args)
{
	if (client)
	{
		...
		char arg[MAX_NAME_LENGTH], arg2[128];
		GetCmdArg(1, arg, sizeof(arg));
		GetCmdArg(2, arg2, sizeof(arg2));

		char target_name[MAX_TARGET_LENGTH];
		int[] players_list = new int[MAX_CLIENTS];
		int players_count, targetclient;
		bool send;
...
{
	char sMsg[24];
	int iRadnomVal = GetRandomInt(1, 12);
	...
	{
		int[] iClients = new int[MAX_CLIENTS];
		int iClient[2];
		int iCount;
...
public void OnCVarChange(ConVar convar_hndl, const char[] oldValue, const char[] newValue)
{
	...
	if (!g_fCvarTimer && g_hxMsg != null)
	{
		delete(g_hxMsg);
		g_hxMsg = null;
	}
	else if (g_fCvarTimer && g_hxMsg == null)
		SetupTimer();
}
