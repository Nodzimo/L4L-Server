...
- `l4l install`
  - Устанавливает через **SteamCMD** чистый сервер для указанной сборки
...
- `l4l restart`
  - Перезагружает указанный сервер, пример:
  - `l4l restart vanilla` перезагрузит **ВСЕ** ванильные сервера с 5-секундной задержкой
- `l4l status`
...
- `l4l maps`
  - Команда-конструктор для подключения (линкования) наборов кастомных карт на сервера, примеры:
  ...
  - Все наборы карт находятся в `/home/steam/l4l/shared/maps/`, там же можно узнать их названия для использования в команде.
- `l4l geo`
...
- `l4l clean`
  ...
  - Эти старые библиотеки загружаются (через **SteamCMD**) каждый раз после установки сервера, поэтому команду нужно вводить каждый раз после свежей переустановки.
